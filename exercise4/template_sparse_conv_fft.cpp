...
template<class T> struct sparse_vec {
	...
	T get_val(int ind) const {
		if (duplets.isempty()) {
			...
			return _get_val(0,duplets.size(), ind);
		}
	}
 	
 	T _get_val(int low, int height, int x) {
 		if (height < low) {
 			return 0;
 		}
 		int mid = (height + low)/2;
 		if (duplets[mid].ind == x) {
 			return duplets[mid].val;
 		}
 		if (duplets[mid].ind < x) {
 			return _get_val(mid+1, height, x);
 		}
 		if (duplets[mid].ind > x) {
 			return _get_val(low, mid-1, x);
 		}
 	}
...
int main() {
	...
	// ICH
 	sparse_vec<double> a(4);
 	sparse_vec<double> b(4);
 	a.append(0,1);
 	a.append(1,2);
 	a.append(2,3);
 	a.append(3,4);
 	...
 	b.append(2,2);
 	b.append(3,1);
 	auto l = sparse_vec<double>::cwise_mult(a,b);
 	cout << "asdf" << l.to_string() << endl;
